			...
			<div className={"flex flex-col w-full"}>
				...
				<section className={"w-full h-fit mb-8"}>
					<div className={"flex flex-col w-full mb-8"}>
						...
						<div className={"w-full h-fit p-4 text-sm bg-gray-300 dark:bg-darkSecBG rounded-md text-lightSecondary dark:text-darkSecondary"}>
							<span
						...
						<div className={"flex items-start justify-between"}>
							<span className={"text-sm py-4 block text-lightSecondary dark:text-darkSecondary"}>
								...
								Blinker uses visual cues such as dropping an eye emoji from the center of your screen (default).
							</span>
							<button onClick={handleState} className={`m-4 flex items-center ${status ? 'justify-end' : 'justify-start'} duration-300 transition-all ease-in p-1 border-2 ${status ? 'border-gray-600 dark:border-[#8fbc8f]' : 'border-gray-600'} rounded-full`} style={{aspectRatio: "2/1", height: "1.75rem"}}>
								<div className={`aspect-square h-full rounded-full ${status ? 'bg-blue-500 dark:bg-darkComp' : 'bg-gray-600'}`}></div>
							</button>
						</div>
