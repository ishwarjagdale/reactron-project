			...
			<div className={"flex flex-col w-full"}>
				...
				<section className={"w-full h-fit mb-8"}>
					<div className={"flex flex-col w-full mb-8"}>
						...
						<Timeline callAppUsage={(timeR) => getAppUsages(timeR)} range={timeRange} data={screenLogs} epoch={getEpoch()} />
					</div>
				</section>
							...
							<ul className={"flex flex-col w-2/3 px-4"} style={{height: "240px", overflowY: "auto"}}>
								{
									appUsages.length ? appUsages.map((k, i) => {
										...
										return <li key={i.toString()} id={k.path} onMouseLeave={handleHover} onMouseOverCapture={handleHover}
											...
											<span title={k.path}
												className={"overflow-clip max-w-xs whitespace-nowrap overflow-ellipsis font-semibold text-sm"}>{k.name} <span className={"italic font-normal opacity-75"}>{k.title && k.name && "-"} {k.title}</span> </span>
